...
async function loadClients() {
  ...
  try {
    ...
    hide($('clients-empty'));
    clientsData.forEach(c => list.appendChild(renderClientCard(c)));
  } catch {
    show($('clients-empty'));
    $('clients-empty').querySelector('p').textContent = 'Failed to load clients. Is the server running?';
  }
}
...
function renderClientCard(client) {
  ...
  const stageClass = `stage-${client.relationshipStage || 'prospect'}`;
  const interests = (client.productInterests || []).join(', ') || 'Not specified';

  card.innerHTML = `
    <div class="client-card-header">
      <div class="client-avatar">${esc(initials)}</div>
      <div>
        <div class="client-card-name">${esc(client.name)}</div>
        <div class="client-card-company">${esc(client.company || '')}</div>
      </div>
    </div>
    <div class="client-card-body">
      <div class="client-badges">
        <span class="badge-aum">${esc(client.aumTier || 'high-net-worth')}</span>
        <span class="badge-risk">${esc(client.riskProfile || 'moderate')}</span>
        <span class="badge-stage ${stageClass}">${esc(client.relationshipStage || 'prospect')}</span>
      </div>
      <div class="client-interests"><strong>Interests:</strong> ${esc(interests)}</div>
      ${client.advisorNotes ? `<div class="client-interests"><strong>Notes:</strong> ${esc(client.advisorNotes.slice(0, 80))}${client.advisorNotes.length > 80 ? '…' : ''}</div>` : ''}
    </div>
    <div class="client-card-actions">
      <button class="btn-brief" data-id="${esc(client.id)}">📋 Pre-Meeting Brief</button>
      <button class="btn-edit-client" data-id="${esc(client.id)}">Edit</button>
      <button class="btn-delete-client" data-id="${esc(client.id)}" title="Delete client">🗑</button>
    </div>
  `;

  card.querySelector('.btn-brief').addEventListener('click', () => generateBrief(client.id));
  card.querySelector('.btn-edit-client').addEventListener('click', () => openClientForm(client));
  card.querySelector('.btn-delete-client').addEventListener('click', () => deleteClient(client.id, client.name));
...
function openClientForm(client = null) {
  ...
  $('client-company').value = client?.company || '';
  $('client-email').value = client?.email || '';
  $('client-phone').value = client?.phone || '';
  $('client-aum').value = client?.aumTier || 'high-net-worth';
  $('client-risk').value = client?.riskProfile || 'moderate';
  $('client-stage').value = client?.relationshipStage || 'prospect';
  $('client-interests').value = (client?.productInterests || []).join(', ');
  $('client-notes').value = client?.advisorNotes || '';
  show($('client-modal'));
  $('client-name').focus();
}
...
$('btn-add-client').addEventListener('click', () => openClientForm());
$('btn-cancel-client').addEventListener('click', closeClientForm);
$('btn-close-client-modal').addEventListener('click', closeClientForm);
$('client-modal').addEventListener('click', e => { if (e.target === $('client-modal')) closeClientForm(); });
...
$('client-form').addEventListener('submit', async e => {
  ...
  const payload = {
    ...
    aumTier: $('client-aum').value,
    riskProfile: $('client-risk').value,
    relationshipStage: $('client-stage').value,
    productInterests: $('client-interests').value.split(',').map(s => s.trim()).filter(Boolean),
    advisorNotes: $('client-notes').value.trim(),
  };
...
async function generateBrief(clientId) {
  ...
  $('brief-modal-title').textContent = `Pre-Meeting Brief — ${client?.name || ''}`;
  show($('brief-modal'));
  show($('brief-loading'));
  hide($('brief-content'));
  hide($('btn-copy-brief'));
  ...
  try {
    ...
    hide($('brief-loading'));
    $('brief-content').innerHTML = `<p style="color:#C2410C">Error: ${esc(err.message)}</p>`;
    show($('brief-content'));
  }
}
...
function renderBrief(b) {
  const list = items => items?.length
    ? `<ul class="brief-list">${items.map(i => `<li>${esc(i)}</li>`).join('')}</ul>`
    : '<p class="brief-text" style="font-style:italic;color:var(--text-muted)">None identified</p>';

  const talkingPoints = (b.talkingPoints || []).map(tp =>
    `<div class="brief-talking-point"><strong>${esc(tp.point)}</strong><span>${esc(tp.rationale)}</span></div>`
  ).join('');

  $('brief-content').innerHTML = `
    <div class="brief-section">
      <div class="brief-section-title">Meeting Objective</div>
      <div class="brief-objective">${esc(b.meetingObjective || '')}</div>
    </div>
    <div class="brief-section">
      <div class="brief-section-title">Client Snapshot</div>
      <p class="brief-text">${esc(b.clientSnapshot || '')}</p>
    </div>
    <div class="brief-section">
      <div class="brief-section-title">Relationship History</div>
      <p class="brief-text">${esc(b.relationshipHistory || '')}</p>
    </div>
    <div class="brief-section">
      <div class="brief-section-title">Key Themes</div>
      ${list(b.keyThemes)}
    </div>
    <div class="brief-section">
      <div class="brief-section-title">Talking Points</div>
      <div style="display:flex;flex-direction:column;gap:.5rem">${talkingPoints || '<p class="brief-text" style="font-style:italic;color:var(--text-muted)">None</p>'}</div>
    </div>
    <div class="brief-grid">
      <div class="brief-section">
        <div class="brief-section-title">Risks to Watch</div>
        ${list(b.risksToWatch)}
      </div>
      <div class="brief-section">
        <div class="brief-section-title">Recommended Products</div>
        ${list(b.recommendedProducts)}
      </div>
    </div>
    <div class="brief-section">
      <div class="brief-section-title">Suggested Questions</div>
      ${list(b.suggestedQuestions)}
    </div>
    <div class="brief-section">
      <div class="brief-section-title">Suggested Next Step</div>
      <p class="brief-text">${esc(b.nextStepSuggestion || '')}</p>
    </div>
  `;

  // Store plain text for copy
  $('btn-copy-brief').dataset.text = [
    `MEETING OBJECTIVE: ${b.meetingObjective}`,
    `\nCLIENT SNAPSHOT\n${b.clientSnapshot}`,
    `\nRELATIONSHIP HISTORY\n${b.relationshipHistory}`,
    `\nKEY THEMES\n${(b.keyThemes || []).map(t => `  • ${t}`).join('\n')}`,
    `\nTALKING POINTS\n${(b.talkingPoints || []).map(tp => `  • ${tp.point}\n    → ${tp.rationale}`).join('\n')}`,
    `\nRISKS TO WATCH\n${(b.risksToWatch || []).map(r => `  • ${r}`).join('\n')}`,
    `\nRECOMMENDED PRODUCTS\n${(b.recommendedProducts || []).map(p => `  • ${p}`).join('\n')}`,
    `\nSUGGESTED QUESTIONS\n${(b.suggestedQuestions || []).map(q => `  • ${q}`).join('\n')}`,
    `\nNEXT STEP: ${b.nextStepSuggestion}`,
  ].join('\n');
}

$('btn-close-brief-modal').addEventListener('click', () => hide($('brief-modal')));
$('brief-modal').addEventListener('click', e => { if (e.target === $('brief-modal')) hide($('brief-modal')); });
$('btn-copy-brief').addEventListener('click', () => {
  const text = $('btn-copy-brief').dataset.text || '';
  navigator.clipboard.writeText(text).then(() => toast('Brief copied!')).catch(() => toast('Copy failed'));
});
